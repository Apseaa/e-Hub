                ...
                @foreach ($android_table as $android_table)
                    <div class="justify-content-center">
                        <div class="entry-header d-flex">
                            <div class="col entry-header-inner d-flex justify-content-between align-items-center">
                                <div>
                                    <h2 class="entry-title mb--0 mt--0">
                                        <a href="" class="link-dark">{!! $android_table->title !!}</a>
                                    </h2>
                                </div>
                                <div class="col text-end">
                                <p class="text-muted">Category: {!! $android_table->category !!}</p>
                                <div class="tagify border-0 float-end">
                                    @foreach (explode(',', $android_table->tags) as $tag)
                                        <span
                                            class="align-text-bottom tagify__tag">{{ str_replace(['[', ']', 'value', '{', '}', '"', '"', ':'], '', $tag) }}</span>
                                    @endforeach
                                </div>
                                </div>
                            </div>
                        ...
                        <div class="entry-meta mt--xs text-secondary">
                            ...
                            <span class="meta-item meta-date"><span class="updated">{!! $android_table->created_at !!}</span></span>
                        </div>
